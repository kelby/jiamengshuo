class ActivitiesController < ApplicationController
  ...
  def index
    ...
    @to_my_request = PublicActivity::Activity.where(key: ["snippet.approve", "snippet.refuse"]).where(recipient_id: current_user.id).where.not(owner_id: current_user.id).page(params[:to_my_request]).per(15).includes(:owner,:trackable,:recipient).order("created_at DESC")
