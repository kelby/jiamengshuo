class ActivitiesController < ApplicationController
  ...
  def index
    @to_my_resource = PublicActivity::Activity.where(key: ["keeper_topic.keep", "marker_topic.mark", "reply.create", "comment.create", "liker_comment.like_it"]).where.not(owner_id: current_user.id).page(params[:page])
